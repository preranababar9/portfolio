    ...
    <section id="work" className="bg-card py-16 border-t border-stroke/50 px-6 sm:px-10 md:px-16 lg:px-20">
      ...
      <div className="divide-y divide-stroke/50">
        {experiences.map((exp, i) => (
          <div key={i} className="py-8 grid md:grid-cols-[1fr_3fr] gap-6 group">

            {/* Left — index + period */}
            <div className="flex md:flex-col gap-4 md:gap-2">
              <span className="text-muted/40 text-sm font-medium tabular-nums">
                {String(i + 1).padStart(2, '0')}
              </span>
              <span className="text-accent md:text-lg font-semibold uppercase tracking-wide">
                {exp.period}
              </span>
            </div>
            ...
            <div>
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mb-3">
                <h3 className="text-heading font-bold md:text-xl text-lg">{exp.role}</h3>
